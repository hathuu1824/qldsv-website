...
if (empty($faculty_id)) {
    ...
    if ($first_f = mysqli_fetch_assoc($get_first_faculty)) {
        ...
        header("Location: student.php?faculty=" . $faculty_id);
        exit();
    }
}
...
if ($faculty_id !== '') {
    $sql_sv .= " AND p.faculty_id = '$faculty_id'";
}

$sql_sv .= " ORDER BY a.code ASC";
...
$sql_max_code = "SELECT code FROM account WHERE role = 'student' AND code LIKE 'SV%' ORDER BY code DESC LIMIT 1";
...
$next_code_string = "SV" . str_pad($next_id, 2, "0", STR_PAD_LEFT);
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
        ...
        <div class="table-container">
            <div class="table-card">
                <div class="table-title">
                    <h3>Tổng số sinh viên: <span class="sv-count"><?= $total_students ?></span></h3>
                    <div class="table-btn">
                        <form action="" method="GET" class="search-form">
                            <input type="text" name="search" placeholder="Tìm tên hoặc mã SV..." 
                                value="<?= htmlspecialchars($search) ?>">
                            <button type="submit" class="btn-search">Tìm</button>
                        </form>
                        <button class="btn-add" onclick="openAddModal()">+ Thêm sinh viên</button>
                    </div>
                </div>
                <div class="sv-table">
                    <table>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div style="display: flex; gap: 5px;">
                                            ...
                                            <a href="delete/delete_student.php?id=<?= $row['account_id'] ?>" class="btn-delete" onclick="return confirm('Bạn có chắc chắn muốn xóa?')">Xóa</a>
                                        </div>
                                    </td>
                                </tr>
                            <?php 
                            ...
                            ?>
                                <tr>
                                    <td colspan="10" style="text-align: center; padding: 20px; color: #888;">
                                        Không có dữ liệu sinh viên nào để hiển thị.
                                    </td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
    ...
    <div id="addModal" class="modal">
        <div class="modal-content">
            <form action="process/process_student.php" method="POST" enctype="multipart/form-data">
    ...
    <script>
        const nextStudentCode = "<?php echo $next_code_string; ?>";
        ...
        function openAddModal() {
            ...
            if (inputCode) {
                inputCode.value = nextStudentCode;
            }
        }
    </script>
