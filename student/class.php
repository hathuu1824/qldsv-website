...
$selected_sem = isset($_GET['semester']) ? (int)$_GET['semester'] : 0;
?>
...
<body>
                    ...
                    <?php else: ?>
                        <?php foreach($assignments as $index => $assign): ?>
                            <div class="assignment-item" 
                                ...
                                onclick="window.location.href='assignment.php?id=<?php echo $assign['id']; ?>'">
            ...
            <div class="class-calendar">
                ...
                <div class="calendar-list">
                    <?php if(empty($calendars)): ?>
                        <div class="empty-item">Không có lịch học trong tuần này</div>
                    <?php else: ?>
    ...
    <script>
        ...
        function changeAssign(step) {
            const slides = document.querySelectorAll('.assignment-item');
            if (slides.length === 0) return;

            slides[currentAssignIndex].style.display = 'none';

            currentAssignIndex += step;

            if (currentAssignIndex >= slides.length) {
                currentAssignIndex = 0;
            } else if (currentAssignIndex < 0) {
                currentAssignIndex = slides.length - 1;
            }

            slides[currentAssignIndex].style.display = 'block';
        }
        </script>  
</body>
</html>