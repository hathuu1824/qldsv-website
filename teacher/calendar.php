...
// Lấy ID giảng viên từ Session (đã được định nghĩa trong cf_class.php hoặc header)
...
// SQL Calendar: Thay đổi logic từ student_id sang account_id của bảng classes
$sql_calendar = "
    -- 1. Lịch giảng dạy (Lấy từ class_sessions của các lớp giảng viên dạy)
    ...
    -- 2. Lịch thi của các lớp giảng viên phụ trách
    ...
    -- 3. Hạn nộp các bài tập giảng viên đã giao
...
// SQL Lịch sắp tới: Thay đổi sang logic giảng viên
$sql_upcoming = "
    SELECT * FROM (
        SELECT cs.date AS event_date, s.subject_name AS title, 'hoc' AS type 
        FROM class_sessions cs
        JOIN classes c ON cs.class_id = c.id
        JOIN subjects s ON c.subject_id = s.id
        WHERE c.account_id = ? AND cs.date >= CURDATE()
        ...
        JOIN classes c ON es.class_id = c.id
        JOIN subjects s ON c.subject_id = s.id
        WHERE c.account_id = ? AND es.exam_date >= CURDATE()
        ...
        FROM assignments a
        JOIN classes c ON a.class_id = c.id
        WHERE c.account_id = ? AND a.deadline >= NOW()
    ) AS results
...
$stmt_up->bind_param("iii", $lecturer_id, $lecturer_id, $lecturer_id);
$stmt_up->execute();
                ...
                <div class="detail-card" style="padding-bottom: 5px;">
                    ...
                    <div class="detail-list">
                        <div class="legend" style="margin-bottom: 20px;">
                            <p><span class="dot event-hoc"></span> Lịch giảng dạy</p>
                            <p><span class="dot event-thi"></span> Lịch coi thi</p>
