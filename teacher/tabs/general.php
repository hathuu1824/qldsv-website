...
function renderDetailSection($title, $content, $fieldName) {    
    $tabId = 'tab-' . $fieldName; 
    ...
    <div class="detail-content">
        <div class="section-header-row" style="display: flex; justify-content: space-between; align-items: center;">
            <h4>' . $title . '</h4>
            <a href="javascript:void(0)" class="btn-detail" onclick="openClassModal(\'' . $tabId . '\')">Xem chi tiết</a>
        </div>
        <div class="content-preview" id="preview-' . $fieldName . '" style="color: #666; font-size: 0.9em; margin-top: 5px;">
            ' . nl2br(htmlspecialchars($content ?? 'Chưa có thông tin.')) . '
        </div>
    </div>';
}
?>
                ...
                <div class="members-card">
                    <?php while($row = $members->fetch_assoc()): ?>
                        <div class="members-info">
                            <?php 
                                $display_name = !empty($row['full_name']) ? $row['full_name'] : ($row['account_code'] ?? 'S');
